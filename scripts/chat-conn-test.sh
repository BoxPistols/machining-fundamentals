...
OPENAI_RESP=$(curl -s https://api.openai.com/v1/chat/completions \
  ...
  -d '{"model":"gpt-5.4-nano","messages":[{"role":"user","content":"Say OK"}],"max_tokens":10,"temperature":0.1}')
...
# -----------------------------------------------------------------------------
info "=== Test 2: Gemini 3 Flash OpenAI 互換 endpoint ==="
GEMINI_RESP=$(curl -s https://generativelanguage.googleapis.com/v1beta/openai/chat/completions \
  -H "Authorization: Bearer $GEMINI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"gemini-3-flash","messages":[{"role":"user","content":"Say OK"}],"max_tokens":10,"temperature":0.1}')
if echo "$GEMINI_RESP" | jq -e '.choices[0].message.content' >/dev/null 2>&1; then
  CONTENT=$(echo "$GEMINI_RESP" | jq -r '.choices[0].message.content')
  pass "gemini-3-flash OpenAI 互換 endpoint OK: '$CONTENT'"
  GEMINI_COMPAT_OK=1
else
  ERR=$(echo "$GEMINI_RESP" | jq -r '.error.message // "unknown"')
...
if [ "$GEMINI_COMPAT_OK" = "0" ]; then
  ...
  GEMINI_NATIVE=$(curl -s "https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash:generateContent?key=$GEMINI_API_KEY" \
    -H "Content-Type: application/json" \
  ...
  if echo "$GEMINI_NATIVE" | jq -e '.candidates[0].content.parts[0].text' >/dev/null 2>&1; then
    ...
    fail "Gemini 3 Flash ネイティブも失敗 — モデル ID 要再確認"
  fi
fi
...
JA_RESP=$(curl -s https://api.openai.com/v1/chat/completions \
  -H "Authorization: Bearer $OPENAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"model\":\"gpt-5.4-nano\",\"messages\":[{\"role\":\"user\",\"content\":\"$JA_PROMPT\"}],\"max_tokens\":200,\"temperature\":0.3}")
...
SSE_RESP=$(curl -s -N https://api.openai.com/v1/chat/completions \
  -H "Authorization: Bearer $OPENAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"gpt-5.4-nano","messages":[{"role":"user","content":"Count 1 to 3"}],"max_tokens":30,"temperature":0.1,"stream":true}' \
  | head -20)
